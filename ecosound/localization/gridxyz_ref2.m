...
sdev  = 100e-5;               % data std dev (puts errors on dobs)
...
% idx=0;
% r1 = sqrt((xtrue(1)-xh(ref_hyd,1))^2+(xtrue(2)-xh(ref_hyd,2))^2+(xtrue(3)-xh(ref_hyd,3))^2);
% for i=channels
%    idx=idx+1;
%    r = sqrt((xtrue(1)-xh(i,1))^2+(xtrue(2)-xh(i,2))^2+(xtrue(3)-xh(i,3))^2);
%    dobs(idx) = (r-r1)/v+randn*sdev;
% end

dobs=[0.0004903,0.0003868,0.0003783]'; % measured

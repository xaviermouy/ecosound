...
(one pulse train per file). It computes IPI (inter-pulse interval) and spectral
features for each file and saves results to a CSV file along with a figure per clip.
'''
...
## ####################################################################
audio_dir = r'C:\Users\xavier.mouy\Documents\Projects\2025_CNN_Minke_Raick\IPI\partage_XavierMouy_IPI-20260201T211635Z-3-001\exemples-beaux-sons' # folder where the audio files are
...
spectro_nfft=0.128      # FFT window length (sec)
spectro_frame=0.128     # analysis frame length (sec)
spectro_inc=0.04        # frame increment / hop size (sec)
...
bkg_spectral_subtraction = False  # avoid using denoising unless really necessary - use with caution

# For the spectral measurements
freq_min_hz = 50   # overall minimum frequency for spectrogram display and spectral measurements (Hz)
freq_max_hz = 800  # overall maximum frequency for spectrogram display and spectral measurements (Hz)
...
detection_band_fixed = {
    'use': False,
    'freq_min_hz': 100,   # lower bound of the fixed detection band (Hz)
    'freq_max_hz': 400,   # upper bound of the fixed detection band (Hz)
}
...
first_meas = True
# loop through audio files and perform measurements
for file_path in audio_files:
    try:
        filename = os.path.splitext(os.path.basename(file_path))[0]
        print(filename)

        # define and load audio file
        ...
        if len(peaks_sec) > 1:
            ...
            IPI_cv = IPI_std_sec/IPI_mean_sec #  coefficient of variation (CV) - measure of dispersion
        ...
        print('Issue happened with this file')
